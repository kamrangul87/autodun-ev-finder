...
import type { Station } from "../types/stations"; // <- your existing type
...
export interface StationDrawerProps {
  ...
  /** optional: shows distance & improves directions */
  userLocation?: { lat: number; lng: number } | null;
}

/** small helpers (local-only; no imports changed elsewhere) */
...
export function StationDrawer({
  ...
  const totalConnectors = Array.isArray(station.connectors)
    ? station.connectors.reduce((sum, c) => sum + (c?.quantity ?? 0), 0)
    : 0;
  ...
  const card = (
    <>
      {/* backdrop only on small screens (keeps desktop map fully interactive) */}
      ...
      <div ref={cardRef} className="sd-card" role="dialog" aria-modal="false" aria-label="Station details">
        ...
        <div className="sd-head">
          <div className="sd-title">
            ...
            {distanceKm !== null && (
              <div className="sd-distance">{distanceKm < 1 ? `${Math.round(distanceKm * 1000)} m` : `${distanceKm.toFixed(1)} km`}</div>
            )}
          </div>
          <button ref={closeRef} className="sd-iconbtn" onClick={onClose} aria-label="Close">
            ×
          </button>
        </div>
        ...
        <div className="sd-block">
          ...
          {Array.isArray(station.connectors) && station.connectors.length > 0 && (
            <div className="sd-connlist">
              {station.connectors
                ...
                .map((c, i) => (
                  <div key={`${c.type}-${i}`} className="sd-chip" title={`${c.type}${c.powerKW ? ` • ${c.powerKW}kW` : ""}`}>
                    ...
                    <span className="sd-chip-qty">×{c.quantity ?? 1}</span>
              ...
              {station.connectors.length > 5 && (
                <div className="sd-chip sd-more">+{station.connectors.length - 5} more</div>
              )}
            </div>
          )}
        </div>
      ...
      <style jsx>{`
        ...
        /* mobile: turn into compact bottom sheet without blocking map taps above */
...
/** keep default export too (doesn't disturb existing imports) */

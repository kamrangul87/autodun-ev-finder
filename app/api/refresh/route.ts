 ...
 * If KV env vars / pkg aren't present, we simply skip persistence.
 */
async function loadKV() {
  try {
    // Only load if env hints KV is configured
...
export async function GET(req: NextRequest) {
  try {
    // Determine a base URL we can call ourselves with
    const base =
      process.env.NEXT_PUBLIC_BASE_URL?.replace(/\/$/, '') ||
      req.nextUrl.origin;

    // A small grid over Greater London (tweak later)
    ...
    // Merge by OCM ID (fallback to lat,lon key)

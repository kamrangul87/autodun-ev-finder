...
export default function EVFinder() {
  ...
  async function loadStations() {
    ...
    try {
      ...
      if (conn) params.set("conn", conn); // omit ?conn= when “Any”
  ...
  return (
    <div className="container py-10">
      ...
      <p className="text-sm text-gray-500">Search by UK postcode or use your location.</p>

      {/* Top controls */}
      <div className="grid md:grid-cols-[2fr_1fr_1fr_1fr] gap-3 mt-3">
        <input
          className="card"
        ...
        {/* NEW: heatmap toggle */}
        <button
          className={`btn ${mode === 'heat' ? 'bg-autodun-green text-white' : 'bg-white border'}`}
          onClick={() => setMode(m => (m === 'markers' ? 'heat' : 'markers'))}
        >
          {mode === 'heat' ? 'Heatmap On' : 'Heatmap Off'}
        </button>

        {/* If you still want Refresh, add another row/button below or swap with the toggle */}
      ...
      <div className="grid md:grid-cols-4 gap-4 mt-4">
        ...
        <div className="md:col-span-3">
          <div className="card p-0 overflow-hidden">
            {center ? (
              <Map center={center} stations={safeStations} mode={mode} />
            ) : (
          ...
          <div className="mt-4 card">
            <h3 className="font-bold text-lg mb-2">Results ({safeStations.length})</h3>
            <ul className="divide-y">
              {safeStations.map((s: any) => (

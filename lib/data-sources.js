...
async function fetchOpenCharge(apiKey, lat = 51.5074, lng = -0.1278, distanceKm = 50, maxResults = 500) {
...
export async function fetchStations(lat = 51.5074, lng = -0.1278, distanceKm = 50, sourceOverride = null, maxResults = 500) {
  ...
  try {
    ...
    switch (sourceUpper) {
      case 'OPENCHARGE':
      case 'OCM':
        result = await fetchOpenCharge(process.env.OCM_API_KEY, lat, lng, distanceKm, maxResults);
        break;
      case 'STATIC':

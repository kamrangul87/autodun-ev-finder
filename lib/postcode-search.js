  ...
  try {
    ...
    if (data.status === 200 && data.result) {
      ...
      if (isOutsideUK(lat, lng)) {
        debugLog("[postcodes.io] Result outside UK bounds, clamping");
...
export async function searchLocation(query) {
  ...
  debugLog(`[searchLocation] Query: "${query}"`);

  let result = await searchPostcodesIO(query);
  if (result) {
    debugLog("[searchLocation] Found via postcodes.io");
    return result;
  }

  debugLog("[searchLocation] Falling back to UK-biased geocoding");
  ...
  debugLog("[searchLocation] Found via UK-biased geocoding");
  return {
    ...geocodeResult,
    source: "nominatim",
